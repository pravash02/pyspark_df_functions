...
class FileMethods:
    ...
    def extract_source_file_data(self, spark, object_path='standard_file_valid_fixedlen_text_file/source_file.txt',
                                 data_has_newline='N'):
    ...
    def apply_schema_to_fixedlen_data(self, spark, df_record, record_identifier, nums_char_skip=None,
                                       input_file_dml=None):
        if input_file_dml is None:
            dict_dml = json.loads(self.get_schema('standard_file_valid_fixedlen_text_file/input_source_file_schema.json'))
        else:
            dict_dml = json.loads(self.get_schema(input_file_dml))

        df_record.createOrReplaceTempView('temp')
        # special char replace
        # df_record = spark.sql("select regexp_replace(value, '[^\x20-\x7E]', ' ') value from temp")

        df_record.createOrReplaceTempView('record')

        var_rec_start_index = nums_char_skip + 1
        record_select_expr = ""

        for row in dict_dml[record_identifier]:
            substr_with_comma = "trim(substring(value," + str(var_rec_start_index) + "," + row['size'] + "))"
            # handle string types
            record_select_expr = record_select_expr + " cast(" + substr_with_comma + " as " + row['type'] + ") as " + row['name'] + ","

            var_rec_start_index += int(row['size'])

        print('record_select_expr -', record_select_expr)
    ...
    def read_header(self, spark, df, header_filter=None, input_file_dml=None):
        ...
        df_header_schema = self.apply_schema_to_fixedlen_data(spark, df_header, 'H', nums_char_skip, input_file_dml)

    def read_trailer(self, spark, df, trailer_filter=None, input_file_dml=None):
        nums_char_skip = 1
        df_trailer = df.filter(trailer_filter)
        df_trailer_schema = self.apply_schema_to_fixedlen_data(spark, df_trailer, 'T', nums_char_skip, input_file_dml)

    def read_data(self, spark, df, data_filter=None, input_file_dml=None):
        nums_char_skip = 1
        df_data = df.filter(data_filter)
        df_data_schema = self.apply_schema_to_fixedlen_data(spark, df_data, 'D', nums_char_skip, input_file_dml)
